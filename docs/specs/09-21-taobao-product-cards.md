...
| 决策点 | 选择 | 理由 |
| --- | --- | --- |
| 淘宝接口形态 | 第三方聚合中转 **Just One API**：商品搜索 V2 + 商品详情 **V6**，`token` 查询参数鉴权 | 不需要淘宝开放平台的企业资质和淘宝客备案；字段与错误码见 [09-21-justoneapi-taobao-ssot.md](./09-21-justoneapi-taobao-ssot.md)（下称 SSOT）。详情换过两次：V3 → V8 为省钱（¥0.6 → ¥0.2），V8 → V6 为字段（V6 是供应商标记的推荐版本，多给运费与券后价），见 SSOT 第 4.3 节 |
...
lib/commerce/cards.ts：逐件补全（并发 2 + 单件失败隔离 + 24h 缓存）
        ↓
lib/commerce/taobao.ts：搜索 V2 定位 item_id → 详情 V6 取标题、主图与店铺（格式见 SSOT）
        ↓
...
- 两个方法对应 SSOT 的 `GET /api/taobao/search-item-list/v2`（搜索）与 `GET /api/taobao/get-item-detail/v6`（详情），鉴权为 `token` 查询参数，`page` 固定传 `1`。
- 详情接口一次只接一个 `itemId`，**接口就是单品形状**，不做 `getItemDetails(ids)` 这种假批量。
- 详情映射按**嵌套优先、平铺兜底**：V6 的字段在 `item` / `seller` 下、`itemId` 是字符串，V8/V3 是平铺的 `num_iid`（数字）、`item_imgs`（裸地址字符串或 `{url}`）。只认一种不会报错，只会静默退化成「空标题 + 空图」——见 SSOT 第 4.2 / 4.3 节。
...
3. **图片**：详情接口的 `images[0]`（上游是 `item.images[]`，SSOT 6 节；不是 `item.detailImages[]`，那是详情长图）。详情失败时回退搜索结果的 `picUrl`，卡片标 `detailLevel: "search"`；两者都没有就出无图卡片，**不换商品、不用别的图凑**。
4. **链接**：优先用详情接口的 `detail_url`；**V6 没有这个字段，所以现在恒走拼链接那条路**——按**详情自己的** `itemId` 拼 `https://item.taobao.com/item.htm?id={item_id}`（上游若把请求导到别的商品，详情里的 ID 才是真正拿到数据的那一件）。详情失败时按搜索结果的 `item_id` 拼同一个模板，卡片标 `detailLevel: "search"`。连 `item_id` 都没有才不出卡并计入 `failed`。任何情况下不允许用 `s.taobao.com/search?q=` 或广告位 `auctionURL` 充当商品链接。
...
| 详情 **V6** 单次调用 | **未实测**：2026-09-24 换到 V6 时只用官方公开示例核了字段形状，没有用我们的 token 真调过。第一次真调要盯 SSOT 第 10 节第 1 条那几项（V8 当年也是这么换的，同样没真调过就上线了） |
...
2. **详情接口用哪个版本**：2026-09-24 已定 V6（供应商标记的推荐版本，带券后价与运费），代价是结构变成嵌套、没有 `detail_url`、**单价未核**。映射层按「嵌套优先、平铺兜底」写，所以回退到 V8 只改 `DETAIL_PATH` 一行（SSOT 第 4.3 节）。
3. **一轮的成本要重新对账**：换到 V6 之前是约 ¥5.8/轮（8 次搜索 + 8 次详情，SSOT 第 8.2 节），那是按 V8 的 ¥0.2/次算的，**V6 的单价我们没看过**。对完账再决定要不要压成本；两个更大的杠杆都还没动——**不调详情**（搜索结果本身就够拼一张卡，代价是标题被截断、主图是竖版缩略图）和**降 `TAOBAO_CARD_LIMIT`**。两个都会改变用户看到的东西，所以留着等明确要做再动。
...
7. 接口格式本身没确认的项（详情的 V6 真调、V6 单价、天猫商品在 V6 下的字段差异、搜索 V2 单价、搜索每页条数、QPS 上限）记在 SSOT 第 10 节，不在这里重复。
