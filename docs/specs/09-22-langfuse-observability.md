...
│  │  ├─ xhs_search_feeds              tool
│  │  ├─ xhs_get_feed_detail           tool   ← 45s 的那次一眼可见
│  │  └─ …
│  ├─ pi.turn.1                        agent
│  └─ …
...
| ↳ `xhs_get_feed_detail` 失败 | 23 | 1404.1s | 102.5% | 1035.0s | 75.6% |
| ↳ `xhs_search_feeds` 失败 | 2 | 90.0s | 6.6% | 90.0s | 6.6% |
...
`xhs_search_feeds` 成功时能拿到 20 条笔记，26KB 全文进 trace（超过 SSE 的 2400 字预览上限）——正文完整性这一条也在真实链路上确认了。
