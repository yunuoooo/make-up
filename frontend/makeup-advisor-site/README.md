...
这是腾讯云 CloudBase 静态 Demo 的可构建源码：

- 在线地址：<https://personal-review-d5fgrb9l4fd8ba3c-1471690350.tcloudbaseapp.com/makeup-advisor/app.html>
- 腾讯云入口：`tencent/index.html` 与 `tencent/main.tsx`
- 主要界面：`components/makeup-advisor-app.tsx`
- 预设推荐逻辑：`lib/advisor.ts`
- 静态资源：`public/`

当前版本是浏览器端静态 Demo：回答由本地预设规则生成，数据保存在 `localStorage`；它尚未调用模型 API，也不会实时运行 `xiaohongshu-makeup-advisor` Skill 或搜索小红书。
...
构建结果位于 `tencent-dist/`。该目录属于生成物，不提交到 Git。
