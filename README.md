...
`/xhs-reads` 把 agent 从小红书读到的**原始内容**摊开看——搜索结果的截断预览、笔记的正文/话题/图组/互动数，以及视频的**口播字幕**（按 `[MM:SS]` 一行一条）。失败的调用同样占一张卡（拒答、空结果、`no-voice` / `no-transcript` / `transcript-failed`），因为「它到底看到了什么」正是答案看起来不对时最要核对的东西。侧边栏有入口。

数据**直接来自 pi 的会话文件**，不是另存的副本：工具返回的原始 payload 本来就存在那儿（下一轮能记得上一轮读过什么，靠的就是它）。所以这个页面不写任何东西，也能看历史会话；会话文件被清理掉之后自然消失。接口是只读的 `GET /api/xhs-reads`。
