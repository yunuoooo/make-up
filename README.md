...
对话界面右边有一条**取数证据**面板（收起时是一条窄边，点箭头展开），把 agent 从小红书读到的**原始内容**摊开看——搜索结果的截断预览、笔记的正文/话题/图组/互动数，以及视频的**口播字幕**（按 `[MM:SS]` 一行一条）。失败的调用同样占一张卡（拒答、空结果、`no-voice` / `no-transcript` / `transcript-failed`），因为「它到底看到了什么」正是答案看起来不对时最要核对的东西。

它**跟着当前对话走**：换对话就跟着换，一轮跑完自动刷新。展开时对话仍在旁边（版面挤一点，不遮住），手机上是盖住的抽屉、入口在标题栏。

数据**直接来自 pi 的会话文件**，不是另存的副本：工具返回的原始 payload 本来就存在那儿（下一轮能记得上一轮读过什么，靠的就是它）。所以它不写任何东西、也能看历史对话；会话文件被清理掉之后自然消失。接口是只读的 `GET /api/xhs-reads`。

`/xhs-reads` 还有一份**跨对话**的独立页面（同一份数据，能一次看所有会话），目前没有界面入口，直接开 URL 用。
